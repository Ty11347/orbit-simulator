...
// 真实的万有引力常数 (国际单位制: m³·kg⁻¹·s⁻²)
const G: f64 = 6.67430e-11;

#[derive(Clone, Copy)]
struct Body {
    mass: f64, sma: f64, ecc: f64, inc: f64, lan: f64, aop: f64, m0: f64,
    parent_index: i32, soi_radius: f64, is_simulated: bool, is_burning: bool,     
}

#[wasm_bindgen]
pub struct PhysicsEngine {
    bodies: Vec<Body>,
    local_positions: Vec<f64>, local_velocities: Vec<f64>,     
    absolute_positions: Vec<f64>, absolute_velocities: Vec<f64>,
    parent_indices: Vec<i32>,
    time: f64,
}

// =====================================================================
// 内部物理运算区块 (不暴露给 JS，不需要 #[wasm_bindgen] 标记)
// =====================================================================
impl PhysicsEngine {
    fn compute_analytical(&self, idx: usize, time: f64) -> (f64, f64, f64, f64, f64, f64) {
        ...
        if body.ecc < 1.0 {
            ...
            let mut f_anomaly = if m.abs() > 50.0 {
            ...
            for _ in 0..15 {
                ...
                if f_prime_e.abs() < 1e-12 { break; }
                
                let step = f_e / f_prime_e;
                f_anomaly -= step;
                if step.abs() < 1e-7 { break; }
            }
            e_anomaly = f_anomaly.clamp(-150.0, 150.0);
            ...
            let r_safe = if r_inst < 1e-6 { 1e-6 } else { r_inst };
        ...
        // 纯净数学坐标系！彻底移除所有 ThreeJS 相关的坐标轴 Swap！
    ...
    fn update_keplerian(&mut self, idx: usize) {
        ...
        let sma = if energy.abs() < 1e-9 { 1e9 } else { -mu / (2.0 * energy) };
...
// =====================================================================
// 对外暴露给 JS/TS 的接口 (必须包含 #[wasm_bindgen] 标记)
// =====================================================================
#[wasm_bindgen]
impl PhysicsEngine {
        ...
        for _ in 0..15000 {
            ...
            for i in 0..sim.bodies.len() {
                if sim.bodies[i].parent_index == c_parent as i32 && sim.bodies[i].soi_radius > 0.0 {
                    ...
                    if dist > soi {
                        ...
                        let rel_v = (dvx*dvx + dvy*dvy + dvz*dvz).sqrt().max(0.01);
            ...
            if parent_soi > 0.0 {
                ...
                if dist_to_parent < parent_soi {
                    let v_local = (sim.local_velocities[target_idx*3].powi(2) + sim.local_velocities[target_idx*3+1].powi(2) + sim.local_velocities[target_idx*3+2].powi(2)).sqrt().max(0.01);
            ...
            let period = if sma > 0.0 { 2.0 * std::f64::consts::PI * (sma.powi(3) / mu).sqrt() } else { 100000.0 };
            let orbit_dt = period / 50.0; 
            let dt = if safe_dt < orbit_dt { safe_dt + 0.05 } else { orbit_dt }.clamp(0.01, 100000.0);
        ...
        for _ in 0..substeps {
            ...
            for idx in 0..count {
                ...
                if self.bodies[idx].is_burning {
                    ...
                    let r = r_sq.sqrt();
                    let f1 = if r < 1e-9 { 0.0 } else { -G * mass / (r_sq * r) };
                    ...
                    let f2 = if next_r < 1e-9 { 0.0 } else { -G * mass / (next_r_sq * next_r) };
